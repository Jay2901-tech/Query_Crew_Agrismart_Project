...
def main(args):
    ...
    try:
        ...
        # Load mapping (PV name -> Official name)
        mapping = load_class_mapping()
        pv_to_official = {v["plantvillage"]: k for k, v in mapping.items()}
        ...
        for i, (pred_pv_name, conf) in enumerate(results):
            # Translate to official name if possible
            official_name = pv_to_official.get(pred_pv_name, pred_pv_name)
        ...
        if os.path.exists(guidance_path) and best_official:
            ...
            if best_official in guidance:
                ...
                print(f"Detected: {info.get('label')}")
                print(f"Precaution: {info.get('guidance')}")
            else:
